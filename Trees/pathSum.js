...
var pathSum = function(root, targetSum, sum=0, path="", res=[]) {
  if (!root) return res;
  ...
  if (!root.left && !root.right) {
    if (sum === targetSum)
      res.push(path.split("->").map(str => parseInt(str)));
    return res;
  }

  path += "->";

  if (root.left) pathSum(root.left, targetSum, sum, path, res);
  if (root.right) pathSum(root.right, targetSum, sum, path, res);
